			...
			<SidebarMenu className="space-y-1">
				{mainMenuItems.map((item) => {
					...
					const isDirectlyActive =
						item.url && item.url !== '' && item.url === currentPath;
						...
						return (
							<SidebarMenuItem key={item.title}>
								<Popover>
									...
									>
										<div className="mb-2 pb-2 border-b border-[var(--sidebar-border)]">
											<div className="flex px-1 items-center gap-2 text-sm font-medium text-[var(--sidebar-foreground)]">
					...
					return (
						<SidebarMenuItem key={item.title}>
							...
							{!isCollapsed && hasSubItems && isOpen && (
								<div>
									<SidebarMenuSub className="ml-4 border-l border-[var(--sidebar-border)] pl-3 space-y-1">
										{item.items?.map((subItem) => {
											const isSubActive = subItem.url === currentPath;
											return (
